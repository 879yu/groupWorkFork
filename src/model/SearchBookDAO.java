...
public class SearchBookDAO extends ConnectionDAO{
	...
	public List<BookLendData> searchBook(String keyword, String radioBtn){
		...
		if(radioBtn.equals("0")) {
			sql = "SELECT * FROM BOOKS LEFT JOIN LEND ON BOOKS.BOOK_ID = LEND.BOOK_ID WHERE BOOKS.TITLE LIKE ? GROUP BY LEND.BOOK_ID";
		} else if(radioBtn.equals("1")) {
			sql = "SELECT * FROM BOOKS LEFT JOIN LEND ON BOOKS.BOOK_ID = LEND.BOOK_ID WHERE BOOKS.ISBN_13 LIKE ? GROUP BY LEND.BOOK_ID";
		}
